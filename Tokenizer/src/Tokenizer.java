...
public final class Tokenizer {
    ...
    private static String nextWordOrSeparator(String text, int position) {
        ...
        if (SEPARATORS.contains(String.valueOf(text.charAt(i)))) {
            while (!endString && i + 1 < text.length()) {
                i++;
                endString = !(SEPARATORS
        ...
        if (endString) {
            nextString = text.substring(position, i);
        } else {
            nextString = text.substring(position);
        }
